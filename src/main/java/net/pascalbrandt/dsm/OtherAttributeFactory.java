...
public class OtherAttributeFactory {
    private static final Logger logger = LoggerFactory.getLogger(OtherAttributeFactory.class);
    ...
    static {
        OTHER_CATEGORITCAL_ATTRIBUTES.put(OTHER_PRETTY_ATTRIBUTE_ALCOHOL_CONSUMPTION, new String[] {
                "None", "Heavy", "Average" });
        OTHER_CATEGORITCAL_ATTRIBUTES.put(OTHER_PRETTY_ATTRIBUTE_TRADITIONAL_MEDICINE,
                new String[] { "Yes", "No" });
        OTHER_CATEGORITCAL_ATTRIBUTES.put(OTHER_PRETTY_ATTRIBUTE_IDENTIFIED_REASON, new String[] {
                "Traditional Remedies", "Poor Adherence", "Treatment Breaks",
                "Social issues impacting Rx", "Malabsorption", "Drug Interactions",
                "Drug toxicity", "Single Dose NVP", "Transmitted Resistance", "Non-Disclosure",
                "Financial" });
        OTHER_CATEGORITCAL_ATTRIBUTES.put(OTHER_PRETTY_ATTRIBUTE_OTHER_COMORBIDITIES, new String[] {
                "Yes", "No" });
        OTHER_CATEGORITCAL_ATTRIBUTES.put(OTHER_PRETTY_ATTRIBUTE_DIARRHEA_OR_VOMITING,
                new String[] { "Yes", "No" });
        ...
        OTHER_ATTRIBUTES_REGA_MAP.put(OTHER_PRETTY_ATTRIBUTE_ALCOHOL_CONSUMPTION,
                OTHER_REGA_ATTRIBUTE_ALCOHOL_CONSUMPTION);
        OTHER_ATTRIBUTES_REGA_MAP.put(OTHER_PRETTY_ATTRIBUTE_TRADITIONAL_MEDICINE,
        ...
        OTHER_ATTRIBUTES_REGA_MAP.put(OTHER_PRETTY_ATTRIBUTE_DIARRHEA_OR_VOMITING,
        ...
        OTHER_ATTRIBUTES_REGA_MAP.put(OTHER_PRETTY_ATTRIBUTE_PARTNER_ON_TREATMENT,
    ...
    public static List<Attribute> getOtherAttributes() {
        ...
        for (String name : OTHER_CATEGORITCAL_ATTRIBUTES.keySet()) {
            attributes.add(new Attribute(name, Arrays.asList(OTHER_CATEGORITCAL_ATTRIBUTES
                    .get(name))));
        }
        ...
        for (String attributeName : selectedOtherAttributes) {
            ...
            if (OTHER_CATEGORITCAL_ATTRIBUTES.containsKey(attributeName)) {
                attributes.add(new Attribute(attributeName, Arrays
                        .asList(OTHER_CATEGORITCAL_ATTRIBUTES.get(attributeName))));
            }
        }
        ...
        if (OTHER_NUMERIC_ATTRIBUTES.contains(attribute.name())) {
            ...
            if (OTHER_ATTRIBUTES_REGA_MAP.containsKey(attribute.name())) {
                AttributeFactory.setSimpleNumericAttributeValue(attribute,
                        OTHER_ATTRIBUTES_REGA_MAP.get(attribute.name()), instance, patient, rs);
            } else {
            ...
            if (OTHER_ATTRIBUTES_REGA_MAP.containsKey(attribute.name())) {
                ...
                if (attribute.name().equals(OTHER_PRETTY_ATTRIBUTE_OTHER_COMORBIDITIES)) {
                    // Other Co-Morbidities
                    setAttributeValueOtherCoMorbidities(attribute, instance, patient, rs);
                }
            }
    ...
    // Other Co-Morbidities
    private static void setAttributeValueOtherCoMorbidities(Attribute attribute, Instance instance,
            Patient patient, RegaService rs) {
