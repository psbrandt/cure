...
public class RegaDAO {
    ...
    public List<Patient> getPatients() throws Exception {
        ...
        Query q = getSession().createQuery(
                "select new net.sf.regadb.db.Patient(patient, max(access.permissions)) "
                        + "from PatientImpl as patient " + "join patient.patientDatasets as patient_dataset "
                        + "join patient_dataset.id.dataset as dataset " + "join dataset.datasetAccesses access "
    ...
    public Date getARTInitiationDate(Integer patientId) {
        ...
        query.append("select t from Therapy as t where t.patient.patientIi = :patientId order by t.startDate");
        ...
        List<Therapy> results = (List<Therapy>) q.list();

        if (results == null || results.size() == 0)
            return null;

        return results.get(0).getStartDate();
    }
    
    public List<TestResult> getListOfTestResultsSortedByDate(Integer testType, Integer patientId, String sortOrder) {
        StringBuffer query = new StringBuffer();

        query.append("select tr from TestResult as tr join tr.test.testType as tt where tt.testTypeIi=" + testType);

        query.append(" and tr.patient.patientIi = :patientId order by tr.testDate " + sortOrder);
    ...
    public Date getLatestTestResultDate(List<Integer> testTypes, Integer patientId) {
        /*
         * select * from regadbschema.test_result as tr join regadbschema.test
         * as t on t.test_ii = tr.test_ii where t.test_type_ii in (45, 46) and
         * tr.patient_ii= 6550 limit 1000
         */
    ...
    public List<Patient> getPatients(Dataset dataset) throws Exception {
        ...
        Query q = getSession().createQuery(
                "select new net.sf.regadb.db.Patient(patient, max(access.permissions)) "
                        + "from PatientImpl as patient " + "join patient.patientDatasets as patient_dataset "
                        + "join patient_dataset.id.dataset as dataset " + "join dataset.datasetAccesses access "
                        + "where dataset = :dataset " + "group by patient order by patient");
